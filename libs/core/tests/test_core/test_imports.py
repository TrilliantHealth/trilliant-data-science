...
from thds.core.imports import try_imports
...
def mock_import_module(mocker: MockFixture) -> MagicMock:
    mock = mocker.patch("thds.core.imports.import_module", autospec=True)
...
def test_try_imports_w_module_and_missing_modules(mock_import_module: MagicMock) -> None:
    mock_import_module.side_effect = ImportError
    with pytest.raises(ImportError) as ex:
        try_imports("missing_module1", "missing_module2", module=TEST_PACKAGE)
...
def test_try_imports_w_module_and_missing_extra(
    ...
    mock_import_module.side_effect = ImportError
    with pytest.raises(ImportError) as ex:
        try_imports("missing_module1", "missing_module2", module=TEST_PACKAGE, extra="dev")
