...
def test_timer_decorator_with_total_context(mock_time):
    ...
    assert tracker.tracked_times["wait"].secs == 3.0
    assert (
        tracker.tracked_times["wait"].perc_of_total(tracker.tracked_times[tracker.total].secs) == 100.0
    )
...
def test_context_tracking_in_loop(num_loops: int, mock_time):
    ...
    assert tracker.tracked_times[tracker.total].perc_of_total(total) == 100.0
    assert tracker.tracked_times["nested_wait_2"].perc_of_total(total) == (2.0 / 14.0) * 100.0
