...
def _set_default(default: object = marshmallow.missing) -> ty.Dict[str, ty.Any]:
    ...
    if default is not marshmallow.missing:
        return dict(dump_default=default, load_default=default, required=False)
...
class PartialField(ty.NamedTuple):
...
class SchemaGenerator:
        ...
        return type(
            ...
            dict(
                apply_field_xfs(
                    ...
                    self._gen_fields(attrs_class, **named_fields),
                ),
                Meta=self._meta,
    ...
    def field(self, **inner_field_kwargs) -> PartialField:
        """Defines a field within the context of an existing Schema and attrs type."""
        return PartialField(inner_field_kwargs)

    def nested(self, **outer_field_kwargs) -> ty.Callable[..., NestedSchemaGenerator]:
        def make_nsg(**fields) -> NestedSchemaGenerator:
            return NestedSchemaGenerator(self, outer_field_kwargs, fields)
