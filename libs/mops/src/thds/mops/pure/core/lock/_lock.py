...
def _write(blob_store: BlobStore, lock_uri: str, lock_bytes: bytes) -> None:
    try:
...
def make_read_lockfile(lock_uri: str) -> ty.Callable[[], ty.Optional[_LockContents]]:
    blob_store = lookup_blob_store(lock_uri)

    def read_lockfile() -> ty.Optional[_LockContents]:
        while True:
...
def _store_and_lock_uri(lock_dir_uri: str) -> ty.Tuple[BlobStore, str]:
...
class LockfileWriter:
    ...
    def mark_acquired(self):
        ...
        self.first_acquired_at = utc_now()
    ...
    def write(self) -> None:
        ...
        lock_bytes = json.dumps(lock_contents, indent=2).encode()
        assert lock_bytes
        ...
        # to acquire the lock.
        _write(self.blob_store, self.lock_uri, lock_bytes)
    ...
    def _write_debug(self, lock_contents: _LockContents) -> None:
        ...
        if self.debug and self.first_acquired_at:
            ...
            try:
                self.blob_store.putbytes(
                    debug_uri,
                    json.dumps(lock_contents, indent=2).encode(),
        ...
        if maybe_lock_contents:
            ...
            if is_released(lock):
                ...
                if acquire_delay:
                    ...
                    acquire_delay = 0.0
                if block is not None and utc_now() > start + block:
                    return None
