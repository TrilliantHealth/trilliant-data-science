...
The purpose of this module is to define leasing semantics that work for an orchestrator/remote invocation
pairing.

> This module was named `lock` until mops 3.25. The on-storage paths (`lock/` under the memo URI,
> containing `lock.json`) keep the old name, because they are shared with every other mops process that
> may coordinate on the same memo URI; renaming them is deferred to a major version.

In particular, the basic requirements are:

1. Exclude other orchestrators on a best-effort basis.
1. Recover from a network partition or other failure (i.e. lease liveness).
1. The remote must be able to maintain lease liveness unconditionally as long as it is running.

The basis for the second requirement is that we can be certain that processes holding the lease will
sometimes die or be terminated. We cannot require human involvement to recover from these scenarios.
Therefore, the lease must be able to 'expire' if not actively kept live.
...
reasonably easy to build a 'guaranteed' lease - a lease where "at most one" holder can exist at the same
time. These are the lease semantics we as software developers are most used to.
...
If you are the first to write to a named file, you win the race for that lease.
...
Even if we were willing to break the Blob Store abstraction in favor of a simpler lease, the third
requirement (for there to be two 'cooperative' owners of the same lease (orchestrator and remote)
complicates any simplistic concept of leasing. Two processes will both need to be able to update the
lease's state without themselves coordinating - and this requires a different notion of leasing than is
easily supported by some off-the-shelf systems for distributed coordination. Even the built-in Locks in
Redis don't provide a natural way to 'share' a lease's liveness updates, once the lease is acquired.
...
- generate a local-only UUID representing our attempt to get the lease
- check a known leasefile
- if it exists and it contains our UUID, we have acquired the lease!
...
- if it does not exist, or if it has expired, we write to the leasefile, with our UUID and a timestamp.
...
The lease algorithm is therefore "last writer wins", and it is theoretically prone to live-lock, if too
many writers keep attempting to write to the lease. In practice, we hope that it will be both fairly rare
that multiple orchestrators simultaneously attempt to acquire the lease, as well as rare that there are
enough writers that live-lock becomes realistically possible.

The remote-side lease liveness comes in when the remote process begins - it can continually update the
leasefile with a new timestamp without interfering with the orchestrator process - they'll just go back
and forth keeping it up to date, overwriting the timestamp but maintaining the same UUID without checking
to confirm anything.

Only the orchestrator will 'release' the lease directly - when the remote function invocation ends and
the Shim returns, the orchestrator will confirm that a result exists and then release the lease.

If the orchestrator dies and the remote eventually dies as well, the lease will go stale/expire, and that
will allow any future callers, or callers who were waiting on the existing invocation, to acquire the
lease and therefore launch their own invocation.
