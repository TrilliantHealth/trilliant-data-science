...
FutureInterpreter = ty.Callable[[ty.Optional[R_0], _LastSeenAt], ty.Union[None, FutureDone[R]]]
# a FutureInterpreter is a function that takes an object R_0 and the time.monotonic() at
# which it was last seen, and returns either None (if the status is still in progress) or
# a FutureDone object that contains the result of type R, or, if the status is failure,
# raises an appropriate Exception.


class _FutureInterpretationShim(ty.Generic[R_0, R]):
    def __init__(self, interpreter: FutureInterpreter[R_0, R]) -> None:
    ...
    def __call__(self, r_0: ty.Optional[R_0], last_seen_at: float) -> bool:
        ...
        try:
            ...
            if not isinstance(interpretation, FutureDone):
                return False  # do nothing and do not unregister - the status is still in progress.

            self.future.set_result(interpretation.result)
...
class UncertainFuturesTracker(ty.Generic[K, R_0]):
    ...
    def create(self, key: K, interpreter: FutureInterpreter[R_0, R]) -> PFuture[R]:
        futshim = _FutureInterpretationShim(interpreter)
        with self._lock:
            if key not in self._keyed_futures_state:
                self._keyed_futures_state[key] = _FuturesState([futshim], last_seen_at=0.0)
    ...
    def update(self, key: ty.Optional[K], r_0: ty.Optional[R_0]) -> None:
        ...
        def check_resolution(fut_state: _FuturesState[R_0], inner_r_0: ty.Optional[R_0]) -> None:
            for futshim in fut_state.futshims:
                if futshim(inner_r_0, fut_state.last_seen_at):
                    # the Future is done, so we can remove it from the list of Futures.
                    fut_state.futshims.remove(futshim)

        if key is not None:
            with self._lock:
                if key not in self._keyed_futures_state:
                    self._keyed_futures_state[key] = _FuturesState(list(), last_seen_at=time.monotonic())
                else:
                    # maintain our ordered dict so we can handle garbage collection of stale Futures.
                    self._keyed_futures_state.move_to_end(key)
                    self._keyed_futures_state[key].last_seen_at = time.monotonic()
        ...
        for futs_state in self._keyed_futures_state.values():
            if futs_state.last_seen_at + self._check_stale_seconds < time.monotonic():
