...
### No tracking of referenced-data lifecycles

`mops` never verifies that the blobs referenced by a memoized result (e.g. its returned `Source`s,
or side-effect outputs named in its return value) still exist - a memo-hit trusts its pickle. If
memos or the data they reference live on TTL'd storage, follow the rules in
link:./storage_tiers.adoc[storage tiers], which exist to make "memo alive, data expired"
unreachable.
