...
## A Source is identified by content, not URI

[WARNING]
====
`mops` passes `Source` arguments *by content hash*. During serialization it
records each source in a hash-keyed map (`hash -> {uri, size}`) and the remote
side resolves every `Source` from that map. This means:

**A `Source` cannot encode information that is derivable only from its URI.**

Two `Source` objects with identical file content but different URIs have the
same hash and are therefore indistinguishable to `mops` — passed together (e.g.
in a `list[Source]`), one silently shadows the other, and both resolve to a
single surviving URI. The classic trap is hive-partitioned parquet where the
partition value (e.g. `data_source=Komodo` vs `data_source=Komodo_Voldemort`)
lives only in the directory path: two rows identical in every in-file column
produce byte-identical files that collide, and one partition's file is dropped.

If a file's identity depends on URI-only information, do one of:

- **encode that information as data inside the file** (preferred — the file is
  then self-describing and can't "lose" the value when downloaded or moved), or
- **pass it explicitly as a separate argument** alongside the `Source`, or
- **pass a raw URI string** instead of a `Source` (opting out of the
...
## Sources and TTL'd storage

A `Source` handed out by a memo-hit is a *reference* to remote bytes; `mops` does not check that
the bytes still exist. When Sources point into TTL'd scratch storage, see
link:./storage_tiers.adoc[storage tiers] for the rules that keep memos from outliving the data
they reference.
