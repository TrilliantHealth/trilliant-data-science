...
Magic is the new & preferred API for core `mops` usage. This ties together a lot of
functionality that has already been available in `mops.pure`, but it provides
...
WARNING: The `pure.magic` decorator and its corresponding configuration APIs are designed
to be used only at the module level, during import, as they modify the global 'magic'
state of your `mops` configuration. For runtime (stack-local) function-wrapping, see
xref:pure-magic-wand[pure.magic.wand].
...
pure.magic.blob_root('adls://lazing/sunday')
----
...
The section headers (`[foo]`, `[foo.bar]`, etc.) are **Python module paths**. Configuration
under `[foo.bar]` applies to all `@pure.magic()` functions in the `foo.bar` module and its
submodules. To target a specific function, use `[module.path.function_name]`.
...
=== Config file specificity and `+__mask+`

Config-file entries follow the same specificity rules as code-level
`+pure.magic.shim/blob_root/pipeline_id+` calls: a more specific path always wins over a
less specific one, regardless of whether the entry came from code or a config file. A
config-file entry at the same path as a code-level entry _will_ override it (since the
config file is loaded after imports), but a package-level config-file entry will **not**
override a function-level entry set by a decorator.

For example, if your code has `+@pure.magic(k8s_shim)+` on `+foo.bar.my_func+`, and your
`+.mops.toml+` has:

.`+.mops.toml+` - this does NOT override the decorator
[source,toml]
----
[foo.bar]
mops.pure.magic.shim = 'subprocess'
----

The decorator's shim wins, because `+foo.bar.my_func+` is more specific than `+foo.bar+`.
Functions in `+foo.bar+` that don't have their own shim set will use `+'subprocess'+`.

To override `+my_func+` specifically, target it by its full path:

.`+.mops.toml+` - targets the exact function
[source,toml]
----
[foo.bar.my_func]
mops.pure.magic.shim = 'subprocess'
...
To override _everything_ under `+foo.bar+` regardless of function-level config, use
`+__mask+`:

.`+.mops.toml+` - masks the entire subtree
[source,toml]
----
[foo.bar]
__mask.mops.pure.magic.shim = 'subprocess'
----

`+__mask+` is useful for development workflows where you want to force an entire module
tree to run locally (or with a different shim) without modifying any code.

== `pure.magic.wand`

> Dynamic/runtime application of `pure.magic`

Because `pure.magic` implements a global registry for configuration, it is designed to be
applied as a decorator directly to function ``def``s at the root of a module. While it will
_work_ when applied as a higher-order function (rather than a decorator), it will cause
spooky action at a distance because of how everything is connected via that global
registry. Think of this like monkey-patching - you _can_ have a different part of your
program completely _replace_ another part of your program at runtime, but _should_ you?

However, different people prefer different approaches, and some prefer a truly dynamic
approach where it is only at runtime that functions have `mops` applied. This was always
possible using `use_runner` and `MemoizingPicklingRunner` manually, but this API was
harder to discover, and crucially, missed a specific user demand for "partially magic"
configurations. The most common use case seems to be "use the magic/global blob_root and
pipeline_id, but use an explicitly-provided shim (which is sized to match the specific
runtime of the function)."

We now support this use case explicitly with `pure.magic.wand` - a higher order function
that captures your explicit config, and then, at the time of later application to a
specific function, fills in any missing configuration from the magic global registry.

In principle this can work with any of the three config types even though the dominant use
case is likely to be dynamic shims.

CAUTION: This function is expected to be called _inside_ other code, at runtime. In other
words, it will always be applied as `pure.magic.wand(...)(func)`, and never to be used for
module-global ``def``s as `@pure.magic.wand`, as it does not create a Magic object and
therefore will not participate in the standard magic configuration in predictable ways.

WARNING: Do NOT use `pure.magic.wand()` on functions that are already decorated with
`@pure.magic()` - this would double-wrap them and cause unexpected behavior. If you need
to override the shim for an already-decorated function at runtime, use the function's
`.shim()` context manager instead:

[source,python]
----
# WRONG - double-wrapping!
local_func = pure.magic.wand("subprocess")(my_magic_func)
result = local_func(args)

# CORRECT - use the context manager
with my_magic_func.shim("subprocess"):
    result = my_magic_func(args)
----
