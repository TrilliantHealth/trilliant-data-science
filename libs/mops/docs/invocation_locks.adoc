...
NOTE: These locks coordinate work _across_ orchestrators — they prevent two separate
processes from racing to compute the same invocation. If you have process-local
concurrent calls to the same `mops` function (multiple threads within one
orchestrator, etc.), the simplest correct pattern is still to wrap the call site with
`core.cache.locking`, `core.lazy.lazy`, or another in-process coordination mechanism.
