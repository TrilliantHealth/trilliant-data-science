...
== Sharing across processes and machines

A shared blob root turns mops into a shared, write-once cache that converges
remote-execution and memoization use cases into a single useful property: any caller
producing a result publishes it to a deterministic location, and any caller (now or later,
local or remote) consuming the same arguments reads it back.

* **Write-once semantics**: same args produce the same `memo_uri`; the first caller
  computes and writes, all later callers read.
* **Cross-machine, cross-process, cross-user sharing**: anyone with access to the blob
  store sees results computed by anyone else. Precomputed results flow downstream by
  being _called for_, not by being explicitly distributed — calling code doesn't need to
  know whether a result was computed two seconds or two months ago, or by which process.
* **Concurrent callers**: when two orchestrators call the same function with the same
  arguments at the same time, mops's link:./invocation_leases.adoc[lease mechanism]
  ensures one computes and the other waits, rather than both racing to recompute.
* **Provenance is free**: the `memo_uri` _is_ the provenance — module + function + args +
  pipeline id, deterministic.

This is the property that no standalone Python cache library provides: the cache is the
namespace where results live, not just a local optimization on top of a function call.
...
1. A blob root (non-optional, user-controlled, has `pure.magic` default).
   `pure.magic` sets the default blob root as `file://$HOME/.mops`.
+
...
* link:peek.adoc[Peek] - ask whether a memoized result exists (and get it) without
  computing one.
* link:storage_tiers.adoc[Storage tiers] - routing memos, intermediates, and final
  outputs to storage with lifecycles that match their roles, and the rules that keep
  memoization correct when intermediate bytes expire.
