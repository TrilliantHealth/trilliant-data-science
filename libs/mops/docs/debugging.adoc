...
├── lock/               # Leasefile coordinating concurrent runs (dir name is historical)
...
* **Different timestamps** = runs started at different times (maybe k8s retry, lease expiry, etc.)

The `invoker_uuid` in the metadata (separate from run_id) tells you about lease ownership:

* **Same invoker_uuid** = same lease holder, but multiple actual executions (e.g., k8s ran multiple pods from the same Job)
* **Different invoker_uuid** = different lease holders raced
...
See link:invocation_leases.adoc[Invocation Leases] for details on lease behavior, timeouts, and lost leases.
...
3. The runtime re-executes your function (k8s retry, lease expiry, etc.)
...
## Blob-not-found on a memo-hit

If a fast memo-hit hands back `Source`s (or output paths) whose downloads then fail with
blob-not-found, the memo has outlived its referenced data - the classic symptom of TTL'd storage
expiring data underneath a live memo. See link:./storage_tiers.adoc[storage tiers] for the rules
that prevent this, and re-run with a fresh output namespace to force recomputation.
