...
Look inside `+~/.mops/mops2-mpf/magic/__main__--fibonacci+`. You'll see 30 directories, each with an odd three-word name with more base64-encoded gibberish after it. This is the SHA256 hash of the arguments to your function - in this case, only `n`. Inside each of these is an `invocation`, a `result`, a `lock` directory (the link:invocation_leases.adoc[invocation lease] - the dir name is historical), and a `result-metadata-<run_id>.txt` file (where `run_id` looks like `2601271523-SkirtBus` - a timestamp plus two random words). The only ones you care about are `invocation` and `result`. For more on the storage structure and debugging, see link:debugging.adoc[Debugging].
...
Want a fresh run? Pass a different `pipeline_id` to `@pure.magic()` (e.g.
`@pure.magic(pipeline_id='try-again')`) and run again — you'll see the green
`+Invoking+` logs come back. The previous results are still on disk under their old
`pipeline_id`, untouched.
